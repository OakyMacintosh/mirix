...
static int find_tool(const char *tool, char *path, size_t path_size) {
    ...
    while (dir) {
        snprintf(path, path_size, "%s\\%s.exe", dir, tool);
...
int find_compiler_and_linker(char *cc_path, size_t cc_size,
    ...
    if (find_tool("cl", cc_path, cc_size) != 0 &&
        find_tool("gcc", cc_path, cc_size) != 0 &&
        find_tool("clang", cc_path, cc_size) != 0)
        return -1;

    if (find_tool("link", ld_path, ld_size) != 0 &&
        find_tool("ld", ld_path, ld_size) != 0)
        return -1;
