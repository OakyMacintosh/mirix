...
static int find_tool(const char *tool, char *path, size_t path_size) {
    ...
    // For IRIX, try common SGI compiler locations
    const char *sgi_paths[] = {
        "/usr/bin",
        "/usr/local/bin",
        "/usr/sgug/bin",
        "/opt/gcc/bin",
    ...
    for (int i = 0; sgi_paths[i]; i++) {
        snprintf(path, path_size, "%s/%s", sgi_paths[i], tool);
...
int find_compiler_and_linker(char *cc_path, size_t cc_size,
    ...
    if (find_tool("cc", cc_path, cc_size) != 0 &&
